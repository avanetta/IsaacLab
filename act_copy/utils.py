        ...
        with h5py.File(dataset_path, 'r') as root:
            for episode_id in tqdm(self.episode_ids):
                ...
                self.cache[episode_id] = {
                    ...
                    **({'qvel': demo['states/articulation/robot/joint_velocity'][()]} if self.velocity_control else {}),
    ...
    def __getitem__(self, index):
        ...
        if self.cache_mode == 'full' and episode_id in self.cache:
            ...
            with h5py.File(dataset_path, 'r') as root:
                ...
                qpos = demo['states/articulation/robot/joint_position'][start_ts:self.context_length + start_ts, :]

                if self.velocity_control:
                    #qvel = demo['states/articulation/robot/joint_velocity'][start_ts:self.context_length + start_ts, :-1]
                    qvel = demo['states/articulation/robot/joint_velocity'][start_ts:self.context_length + start_ts, :]
...
def get_norm_stats(dataset_dir, num_episodes):
    ...
    with h5py.File(dataset_path, 'r') as root:
        for episode_idx in range(num_episodes):
            ...
            qpos = demo['states/articulation/robot/joint_position'][()] # [T_i, nq]
            qvel = demo['states/articulation/robot/joint_velocity'][()]
